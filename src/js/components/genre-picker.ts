...
export class GenrePicker {
  ...
  constructor({ container, userId, onChange = () => {} }: GenrePickerOptions) {
   ...
   */
  setSelected(genreIds: string[]): void {
  ...
  render(): void {
    ...
    const activeInput = this.container.querySelector('.genre-picker-input') as HTMLInputElement | null;
    const hadFocus = activeInput && document.activeElement === activeInput;
    const cursorPos = hadFocus && activeInput ? (activeInput.selectionStart ?? 0) : 0;
    ...
    if (hadFocus) {
      const newInput = this.container.querySelector('.genre-picker-input') as HTMLInputElement | null;
      if (newInput) {
   ...
   */
  private _getFilteredGenres(): Genre[] {
  ...
  private _attachEventListeners(): void {
    ...
    this.container.querySelectorAll('[data-remove-genre]').forEach(btn => {
      btn.addEventListener('click', (e: Event) => {
        e.preventDefault();
        e.stopPropagation();
        const genreId = (btn as HTMLElement).dataset.removeGenre!;
    ...
    this.container.querySelectorAll('[data-genre-id]').forEach(btn => {
      btn.addEventListener('click', (e: Event) => {
        e.preventDefault();
        const genreId = (btn as HTMLElement).dataset.genreId!;
    ...
    this.container.querySelectorAll('[data-suggestion]').forEach(btn => {
      btn.addEventListener('click', async (e: Event) => {
        e.preventDefault();
        const name = (btn as HTMLElement).dataset.suggestion!;
   ...
   */
  private _toggleGenre(genreId: string): void {
   ...
   */
  private _updateSelectionUI(genreId: string, isAdding: boolean): void {
    ...
    if (badgesContainer) {
      ...
      if (genre) {
        if (isAdding) {
          ...
          `;
          const removeBtn = badge.querySelector('[data-remove-genre]');
          ...
          this._sortBadges(badgesContainer as HTMLElement);
        } else {
          // Remove badge
   ...
   */
  private _sortBadges(container: HTMLElement): void {
   ...
   */
  private _removeGenre(genreId: string): void {
    const index = this.selected.indexOf(genreId);
    if (index !== -1) {
      this.selected.splice(index, 1);
      this.render();
   ...
   */
  private async _addSuggestion(name: string): Promise<void> {
    try {
